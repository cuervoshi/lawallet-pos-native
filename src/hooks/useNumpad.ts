...
export interface IUseNumpad {
  ...
  resetAmount: () => void;
  modifyCurrency: (new_currency: AvailableCurrencies) => void;
  setCustomAmount: (
    amount: number,
    currency_value: AvailableCurrencies,
  ) => void;
...
export const useNumpad = (
  ...
  const multiplier = (value: AvailableCurrencies) => 10 ** decimalsToUse(value);
  ...
  const updateNumpadAmount = (new_amount: string) => {
    ...
    CurrenciesList.map(value => {
      if (value === usedCurrency) {
        ...
        formattedAmounts[value] = convertedAmount;
      }
    });
  ...
  const setCustomAmount = (
    ...
    setStrAmount((amount * multiplier(currency_value)).toFixed(0).toString());
  };
